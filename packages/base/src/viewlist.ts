 ...
 * - create_view takes a model and an index and returns a view or a promise for a view for that
 *   model
 ...
 * - the remove callback defaults to just removing the view (e.g., pass in null for the second
 *   parameter)
 * - the context defaults to the created ViewList.  If you pass another context, the create and
 *   remove will be called in that context.
 */
export
class ViewList<T> {
  constructor(create_view: (model: any, index: any) => T | Promise<T>,
              remove_view: (view: T) => void, context) {
    this.initialize(create_view, remove_view, context);
  }

  initialize(create_view: (model: any, index: any) => T | Promise<T>,
             remove_view: (view: T) => void, context) {
   ...
   */
  update(new_models, create_view?: (model: any, index: any) => T | Promise<T>,
         remove_view?: (view: T) => void, context?): Promise<T[]> {
