...
export const PROTOCOL_MAJOR_VERSION = PROTOCOL_VERSION.split('.', 1)[0]
...
export abstract class ManagerBase<T> {
   ...
   */
  display_model(msg: services.KernelMessage.IMessage, model: WidgetModel, options: any = {}):
      Promise<T> {
    return this.create_view(model, options)
        .then(view => this.display_view(msg, view, options))
        .catch(utils.reject('Could not create view', true));
  };
   ...
   */
  abstract display_view(msg: services.KernelMessage.IMessage, view: WidgetView, options: any):
      Promise<T>;
  ...
  create_view(model: WidgetModel, options = {}): Promise<WidgetView> {
    let viewPromise = model.state_change = model.state_change.then(() => {
      return this
          .loadClass(
              model.get('_view_name'), model.get('_view_module'), model.get('_view_module_version'))
          .then((ViewType) => {
            let view: WidgetView =
                new ViewType({model: model, options: this.setViewOptions(options)});
   ...
   */
  handle_comm_open(comm: shims.services.Comm, msg: services.KernelMessage.ICommOpenMsg):
    ...
    if (protocolVersion.split('.', 1)[0] !== PROTOCOL_MAJOR_VERSION) {
      let error = `Wrong widget protocol version: received protocol version '${
          protocolVersion}', but was expecting major version '${PROTOCOL_MAJOR_VERSION}'`;
      console.error(error)
    ...
    return this
        .new_model(
            ...
            data.state)
        .catch(utils.reject('Could not create a model.', true));
  };
  ...
  new_widget(options: WidgetOptions, serialized_state: any = {}): Promise<WidgetModel> {
    ...
    if (options.view_name === undefined || options.view_module === undefined ||
    ...
    if (options.comm) {
      ...
      commPromise = this._create_comm(
          this.comm_target_name, options.model_id, {
            state: {
  ...
  async _make_model(options: ModelOptions, serialized_state: any = {}): Promise<WidgetModel> {
    ...
    let model_promise =
        this.loadClass(options.model_name, options.model_module, options.model_module_version) as
    ...
    if (!ModelType) {
      throw new Error(`Cannot find model module ${options.model_module}@${
          options.model_module_version}, ${options.model_name}`);
    }
   ...
   *
   * @param state - a Javascript object conforming to the application/vnd.jupyter.widget-state+json
   * spec.
   *
  ...
  set_state(state): Promise<WidgetModel[]> {
    ...
    let all_models = this._get_comm_info().then(live_comms => {
      return Promise.all(Object.keys(models).map(model_id => {
        ...
        let decode: {[s: string]: (s: string) =>
                         ArrayBuffer;} = {'base64': utils.base64ToBuffer, 'hex': utils.hexToBuffer};
        let model = models[model_id];
   ...
   */
  protected abstract loadClass(className: string, moduleName: string, moduleVersion: string):
      Promise<any>;
  ...
  protected abstract _create_comm(
      comm_target_name: string, model_id: string, data?: any, metadata?: any): Promise<any>;
