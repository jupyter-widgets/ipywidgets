...
abstract class ManagerBase<T> {
   ...
   */
  display_model(msg: services.KernelMessage.IMessage, model: WidgetModel,
   ...
   */
  abstract display_view(msg: services.KernelMessage.IMessage, view: WidgetView,
                        options: any): Promise<T>;
   ...
   */
  handle_comm_open(comm: shims.services.Comm,
                   msg: services.KernelMessage.ICommOpenMsg): Promise<WidgetModel> {
  ...
  async new_model(options: ModelOptions, serialized_state: any = {}): Promise<WidgetModel> {
    ...
    if (options.model_id) {
      ...
      throw new Error('Neither comm nor model_id provided in options object.' +
  ...
  set_state(state): Promise<WidgetModel[]> {
    ...
    let all_models = this._get_comm_info().then(live_comms => {
      return Promise.all(Object.keys(models).map(model_id => {
        ...
        let decode: { [s: string]: (s: string) => ArrayBuffer; } = {
          'base64': utils.base64ToBuffer,
          'hex': utils.hexToBuffer
        };
        let model = models[model_id];
        ...
        // If the model has already been created, set its state and then return it.
   ...
   */
  protected abstract loadClass(className: string, moduleName: string,
   ...
   */
  protected abstract _create_comm(comm_target_name: string, model_id: string, data?: any,
