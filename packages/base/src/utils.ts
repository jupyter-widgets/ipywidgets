...
export {difference, isEqual} from 'lodash';
 ...
 */
export let assign = (Object as any).assign || function(t) {
...
export function uuid(): string {
  ...
  s[16] =
      hexDigits.substr((s[16] & 0x3) | 0x8, 1);  // bits 6-7 of the clock_seq_hi_and_reserved to 01
 ...
 */
export function resolvePromisesDict(d): Promise<any> {
 ...
 */
export function put_buffers(state, buffer_paths: (string|number)[][], buffers: DataView[]) {
  for (let i = 0; i < buffer_paths.length; i++) {
    let buffer_path = buffer_paths[i];
    // say we want to set state[x][y][z] = buffers[i]
    let obj = state;
    // we first get obj = state[x][y]
    for (let j = 0; j < buffer_path.length - 1; j++) obj = obj[buffer_path[j]];
 ...
 * are removed. If a buffer is a member of an object, that object is cloned, and the key removed. If
 * a buffer is an element of an array, that array is cloned, and the element is set to null. See
 * put_buffers for the meaning of buffer_paths Returns an object with the new state (.state) an
 * array with paths to the buffers (.buffer_paths), and the buffers associated to those paths
 * (.buffers).
 */
export function remove_buffers(state):
    {state: any, buffers: ArrayBuffer[], buffer_paths: (string|number)[][]} {
      ...
      for (let i = 0; i < obj.length; i++) {
        ...
        if (value) {
          if (value.buffer instanceof ArrayBuffer || value instanceof ArrayBuffer) {
            ...
            // only assigned when the value changes, we may serialize objects that don't support
            // assignment
            if (new_value !== value) {
              if (!is_cloned) {
                obj = obj.slice();
  ...
  return {
    state: new_state, buffers: buffers, buffer_paths: buffer_paths
  }
}
