...
export const JUPYTER_WIDGETS_VERSION = '1.0.0';

/**
 * Replace model ids with models recursively.
 */
export function unpack_models(value, manager): Promise<any> {
  let unpacked;
...
export class WidgetModel extends Backbone.Model {
   ...
   * Close model
   *
   * @param comm_closed - true if the comm is already being closed. If false, the comm will be
   * closed.
   *
        ...
        this.state_change =
            this.state_change
                .then(() => {
                  ...
                  return (this.constructor as typeof WidgetModel)
                      ._deserialize_state(state, this.widget_manager);
                })
                .then((state) => {
                  this.set_state(state);
                })
                .catch(utils.reject(
                    `Could not process update msg for model id: ${this.model_id}`, true))
  ...
  sync(method: string, model: WidgetModel, options: any = {}): any {
    // the typing is to return `any` since the super.sync method returns a JqXHR, but we just return
  ...
  send_sync_message(state, callbacks: any = {}) {
    try {
      ...
      this.comm.send(
          {method: 'update', state: split.state, buffer_paths: split.buffer_paths}, callbacks, {},
          split.buffers);
      this._pending_msgs++;
    } catch (e) {
...
export abstract class WidgetView extends NativeView<WidgetModel> {
  /**
   * Public constructor.
   */
...
export namespace JupyterPhosphorWidget {
  export interface IOptions { view: DOMWidgetView; }
}

export class JupyterPhosphorWidget extends Widget {
...
export class DOMWidgetView extends WidgetView {
  ...
  _setElement(el: HTMLElement) {
    ...
    this.el = this.$el[0];
    this.pWidget = new JupyterPhosphorWidget({node: el, view: this});
  }
