...
describe('ManagerBase', () => {
  ...
  describe('comm_target_name', () => {
    it('is \'jupyter.widget\'', () => {
      expect(managerBase.comm_target_name).to.equal('jupyter.widget');
    });
  });

  describe('display_model', () => {
    it('exists', () => {
      expect(managerBase.display_model).to.not.be.undefined;
    });
    it('calls create_view and display_view', async () => {
      ...
      let model = await manager.new_model(modelOptions);
  ...
  describe('setViewOptions', () => {
    it('returns an object', () => {
      expect(managerBase.setViewOptions()).to.deep.equal({});
    });
    it('returns the passed options', () => {
      let options = {a: 1};
      expect(managerBase.setViewOptions(options)).to.deep.equal(options);
    });
  });
  ...
  describe('get_model', () => {
    ...
    it('returns undefined when model is not registered', () => {
      expect(managerBase.get_model('not-defined')).to.be.undefined;
    });
  });
  ...
  describe('new_model', () => {
    it('returns a promise to a model', async () => {
      ...
      expect(model.name).to.be.equal(modelOptions.model_name);
      expect(model.module).to.be.equal(modelOptions.model_module);
    });
    ...
    it('deserializes attributes using custom serializers and handles binary state',
        async () => {
      let manager = managerBase;
      let model = await manager.new_model({
        model_name: 'BinaryWidget',
