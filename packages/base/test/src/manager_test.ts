...
describe('ManagerBase', function() {
  ...
  describe('new_widget', function() {
    ...
    it('rejects if view information is not passed in', async function() {
      ...
      let manager = this.managerBase;
      expect(manager.new_widget(spec))
          .to.be.rejectedWith('new_widget(...) must be given view information in the options.');
    });
    ...
    it('deserializes attributes using custom serializers and handles binary state',
       async function() {
         ...
         let model = await manager.new_model(
             ...
             {array: {dtype: 'uint8', buffer: new DataView((new Uint8Array([1, 2, 3]).buffer))}});
         expect(model.get('array')).to.deep.equal(new Uint8Array([1, 2, 3]));
       });
  ...
  describe('get_state', function() {
    ...
    it('handles the drop_defaults option', async function() {
      let manager = this.managerBase;
      let model = await manager.new_model(this.modelOptions, {value: 50});
      ...
      let expectedState = {
        ...
        'state': {
          'u-u-i-d': {
            ...
            'model_module': 'test-widgets',
            'model_module_version': '1.0.0',
            'state': {'array': {'dtype': 'uint8'}},
            'buffers': [{'data': 'AQID', 'path': ['array', 'buffer'], 'encoding': 'base64'}]
          }
        }
      };
      expect(state).to.deep.equal(expectedState);
      ...
      let state = {
        ...
        'state': {
          'u-u-i-d': {
            ...
            'model_module_version': '1.0.0',
            'state': {'array': {'dtype': 'uint8'}},
            'buffers': [{'data': '010203', 'path': ['array', 'buffer'], 'encoding': 'hex'}]
          }
        }
      };
      let manager = this.managerBase;
