...
describe('WidgetModel', function() {
  before(async function() {
    this.setup = async function() {
      ...
      this.widget =
          new WidgetModel({}, {model_id: 'widget', widget_manager: this.manager, comm: this.comm});
      // Create some dummy deserializers.  One returns synchronously, and the
      // other asynchronously using a promise.
      this.serializeToJSON = sinon.spy(() => {return 'serialized'});
  ...
  describe('constructor', function() {
    ...
    it('can take initial state', function() {
      ...
      expect(widget.attributes).to.deep.equal({...widget.defaults(), a: 1, b: 'b state'});
    });
    ...
    it('sets the widget_manager, id, comm, and comm_live properties', function() {
      ...
      let widgetLive =
          new WidgetModel({}, {model_id: 'widgetLive', widget_manager: this.manager, comm: comm});
  ...
  describe('send', function() {
    ...
    it('sends custom messages with the right format', function() {
      ...
      let widget =
          new WidgetModel({}, {model_id: 'widget', widget_manager: this.manager, comm: comm});
      ...
      expect(send.getCall(0))
          .to.be.calledWithExactly({method: 'custom', content: data1}, callbacks, {}, buffers);
      expect(send.getCall(1))
          .to.be.calledWithExactly({method: 'custom', content: data2}, callbacks, {}, buffers);
    });
  });
  ...
  describe('_handle_comm_msg', function() {
    ...
    it('listens to widget messages', async function() {
      await this.widget.comm._process_msg({content: {data: {method: 'update', state: {a: 5}}}});
    ...
    it('handles update messages', async function() {
      ...
      let state_change =
          this.widget._handle_comm_msg({content: {data: {method: 'update', state: {a: 5}}}});
    ...
    it('updates handle various types of binary buffers', async function() {
      ...
      expect(this.widget.get('c')).to.deep.equal([
        'start', new DataView(buffer2.buffer), {d: new DataView(buffer3.buffer)}
      ]);
    });
  ...
  describe('_deserialize_state', function() {
    ...
    it('deserializes simple JSON state', async function() {
      let state = await this.widget.constructor._deserialize_state(
          {a: 10, b: [{c: 'test1', d: ['test2']}, 20]}, this.manager);
    ...
    it('respects custom serializers', async function() {
      let state = await this.widget.constructor._deserialize_state(
          {times3: 2.0, halve: 2.0, c: 2.0}, this.manager);
  ...
  describe('serialize', function() {
    ...
    it('serializes null values', function() {
      const state_with_null = {a: 5, b: null};
  ...
  describe('get_state', function() {
    ...
    it('drop_defaults is respected', function() {
      this.widget.set('a', 'get_state test');
      expect(this.widget.get_state(true)).to.deep.equal({a: 'get_state test'});
    });
  });
  ...
  describe('sync', function() {
    ...
    it('respects the message throttle', function() {
      ...
      expect(send)
          .to.be
          .calledWith({a: 'sync test'})
          // have the comm send a status idle message
          this.widget._handle_status({content: {execution_state: 'idle'}});
      // check that the other sync message went through with the updated values
      expect(send.secondCall).to.be.calledWith({a: 'another sync test', b: 'change b again'});
    });
  ...
  describe('send_sync_message', function() {
    ...
    it('sends a message', function() {
      this.widget.send_sync_message({a: 'send sync message', b: 'b value'}, {});
      expect(this.comm.send)
          .to.be.calledWith(
              {method: 'update', state: {a: 'send sync message', b: 'b value'}, buffer_paths: []});
    });

    it('handles buffers in messages', function() {
      let buffer = new Uint8Array([1, 2, 3]);
      this.widget.send_sync_message({a: buffer});
      expect(this.comm.send.args[0][0])
          .to.deep.equal({method: 'update', state: {}, buffer_paths: [['a']]});
