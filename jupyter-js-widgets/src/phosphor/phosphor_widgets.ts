...
export
class AccordionLayout extends PanelLayout {
  private _widgets = VectorWithSelection
}
...
class Accordion extends Panel {
  ...
  // Logic that if a widget is collapsed/not collapsed, it updates the
   ...
   */
  addWidget(widget: Widget): void {
    super.addWidget(this._wrapWidget(widget));
  }
   ...
   */
  insertWidget(index: number, widget: Widget): void {
    super.insertWidget(index, this._wrapWidget(widget));
  }

  _wrapWidget(widget: Widget) {
...
// Define the signals for the `Widget` class.
defineSignal(Accordion.prototype, 'currentChanged');
