...
from ..embed import dependency_state, embed_data, embed_minimal_html, embed_snippet
from ..widgets import HBox, IntSlider, IntText, Text, Widget, jslink
from ..widgets import widget as widget_module
from ..widgets import widget_serialization
...
class TestEmbed:
    ...
    def test_escape(self):
        ...
        assert (
            code.find(r"\u003cscript A> \u003cScRipt> \u003c/Script> \u003c!-- --> <b>hi</b>") >= 0
        )
        ...
        assert (
            content.find(r"\u003cscript A> \u003cScRipt> \u003c/Script> \u003c!-- --> <b>hi</b>")
            >= 0
        )
    ...
    def test_snippet(self):
        class Parser(HTMLParser):
            ...
            def handle_starttag(self, tag, attrs):
                ...
                if (
                    tag == "script"
                    and attrs.get("type", "") == "application/vnd.jupyter.widget-state+json"
                ):
                    self.state = "widget-state"
                    self.states.append(self.state)
                elif (
                    tag == "script"
                    and attrs.get("type", "") == "application/vnd.jupyter.widget-view+json"
                ):
