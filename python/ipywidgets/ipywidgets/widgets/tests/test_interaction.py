...
def check_widget(w, **d):
    ...
    for attr, expected in d.items():
        if attr == "cls":
            ...
            assert value == expected, "{}.{} = {!r} != {!r}".format(
            ...
            if isinstance(value, (int, float)):
                tv = type(value)
                te = type(expected)
                assert tv is te, "type({}.{}) = {!r} != {!r}".format(
                    w.__class__.__name__, attr, tv, te
                )
...
def test_state_schema():
    ...
    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../", "state.schema.json")
    ) as f:
        schema = json.load(f)
