...
from .utils import _get_frame, deprecation
...
class Widget(LoggingHasTraits):
    ...
    def widgets():
        ...
        if not (
            ...
            and (frame.f_code.co_name in ("getmembers", "setup_instance"))
        ):
    ...
    def _handle_control_comm_msg(cls, msg):
        ...
        if method == "request_states":
            ...
            cls._control_comm.send(
                dict(method="update_states", states=full_state, buffer_paths=buffer_paths),
                buffers=buffers,
            )

        else:
            raise RuntimeError(
                'Unknown front-end to back-end widget control msg with method "%s"' % method
            )
