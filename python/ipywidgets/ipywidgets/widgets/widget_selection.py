...
class _Selection(DescriptionWidget, ValueWidget, CoreWidget):
    ...
    def _validate_value(self, proposal):
        ...
        try:
            return (
                findvalue(self._options_values, value, self.equals) if value is not None else None
            )
        except ValueError:
...
class ToggleButtons(_Selection):
    ...
    icons = TypedTuple(
        Unicode(), help="Icons names for each button (FontAwesome names without the fa- prefix)."
    ).tag(sync=True)
...
class SelectionSlider(_SelectionNonempty):
    ...
    readout = Bool(True, help="Display the current selected label next to the slider").tag(
        sync=True
    )
    continuous_update = Bool(
...
class SelectionRangeSlider(_MultipleSelectionNonempty):
    ...
    def _validate_index(self, proposal):
        ...
        if len(proposal.value) != 2:
            raise TraitError(
                f"Invalid selection: index must have two values, but is {proposal.value!r}"
            )
        if all(0 <= i < len(self._options_labels) for i in proposal.value):
            return proposal.value
        else:
