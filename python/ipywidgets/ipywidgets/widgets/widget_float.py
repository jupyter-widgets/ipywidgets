...
class FloatLogSlider(_BoundedLogFloat):
    ...
    step = CFloat(
        0.1, allow_none=True, help="Minimum step in the exponent to increment the value"
    ).tag(sync=True)
    orientation = CaselessStrEnum(
        values=["horizontal", "vertical"],
        default_value="horizontal",
...
class _FloatRange(_Float):
    value = Tuple(
        CFloat(), CFloat(), default_value=(0.0, 1.0), help="Tuple of (lower, upper) bounds"
    ).tag(sync=True)
...
class _BoundedFloatRange(_FloatRange):
    step = CFloat(1.0, help="Minimum step that the value can take (ignored by some views)").tag(
        sync=True
    )
    max = CFloat(100.0, help="Max value").tag(sync=True)
    min = CFloat(0.0, help="Min value").tag(sync=True)
